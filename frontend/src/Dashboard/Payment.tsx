...
function Payment() {
    ...
    const handleSubmitPayment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            doc.text(t('centre_desc'), 105, 30, { align: 'center' });
            doc.text(`${t('date')}: ${new Date(date).toLocaleDateString()}`, 105, 35, { align: 'center' });
            ...
            doc.text(`${t('course_id_label')}: ${courseId}`, 20, 80);

            doc.setFontSize(16);
            doc.setFont('helvetica', 'bold');
            doc.text(`${t('amount_paid')}: ${parseFloat(amount).toLocaleString()} DH`, 20, 100);

            if (notes) {
                doc.setFontSize(12);
                doc.setFont('helvetica', 'normal');
                doc.text(`${t('notes_optional')}: ${notes}`, 20, 120);
            }
    ...
    const handleShareReceipt = (payment: PaymentRecord) => {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text(t('centre_desc'), 105, 30, { align: 'center' });
        doc.text(`${t('date')}: ${new Date(payment.PaymentDate).toLocaleDateString()}`, 105, 35, { align: 'center' });
        ...
        doc.text(`${t('student_label')}: ${selectedStudent.UserName}`, 20, 60);
        doc.text(`${t('student_id')}: #${selectedStudent.UserID}`, 20, 70);
        doc.text(`${t('course_id_label')}: ${payment.CourseID}`, 20, 80);

        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text(`${t('amount_paid')}: ${payment.Amount.toLocaleString()} DH`, 20, 100);

        if (payment.Notes) {
            doc.setFontSize(12);
            doc.setFont('helvetica', 'normal');
            doc.text(`${t('notes_optional')}: ${payment.Notes}`, 20, 120);
        }
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-12 mb-8 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-slate-500 font-bold flex items-center gap-2">
                            <DollarSign className="w-4 h-4" /> {t('payment_tracking_desc')}
                        </p>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 border-b border-slate-100">
                                <tr>
                                    <th className="px-8 py-5 text-xs font-black uppercase tracking-widest text-slate-400">{t('student_name')}</th>
                                    <th className="px-8 py-5 text-center text-xs font-black uppercase tracking-widest text-slate-400">{t('total_paid')}</th>
                                    <th className="px-8 py-5 text-right text-xs font-black uppercase tracking-widest text-slate-400">{t('actions')}</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={student.UserID} className="hover:bg-slate-50/50 transition duration-150">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-xs text-slate-400 font-medium">{t('id')}: #{student.UserID}</span>
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex justify-end gap-3">
                                                ...
                                                >
                                                    <Plus className="w-4 h-4" /> {t('record_payment')}
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <h3 className="font-black text-lg text-slate-800">{t('record_payment')}</h3>
                        ...
                        <form onSubmit={handleSubmitPayment} className="p-6 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('student_label')}</label>
                                <div className="font-bold text-slate-900 text-lg">{selectedStudent.UserName}</div>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('amount_dh')}</label>
                                <input
                                    type="number"
                                    required
                                    min="0"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('notes_optional')}</label>
                                <textarea
                                    value={notes}
                                    ...
                                    placeholder={t('add_payment_details')}
                                />
                            </div>
                            <div className="pt-4">
                                <button
                                ...
                                >
                                    {isSubmitting ? t('saving') : <><Save className="w-5 h-5" /> {t('save_payment')}</>}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
                            <div>
                                <h3 className="font-black text-lg text-slate-800">{t('payment_history')}</h3>
                        ...
                        <div className="p-0 overflow-y-auto">
                            {payments.filter(p => p.StudentID === selectedStudent.UserID).length === 0 ? (
                                <div className="p-12 text-center text-slate-400">
                                    ...
                                    <p className="font-bold">{t('no_payments_recorded')}</p>
                                </div>
                            ) : (
                                <table className="w-full text-left">
                                    <thead className="bg-slate-50 sticky top-0">
                                        <tr>
                                            <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-400">{t('date')}</th>
                                            <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-400">{t('amount_paid')}</th>
                                            <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-400">{t('notes_optional')}</th>
                                            <th className="px-6 py-4 text-right text-xs font-black uppercase tracking-widest text-slate-400">{t('receipt')}</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {payments
                        ...
                        <div className="p-6 border-t border-slate-100 bg-slate-50 shrink-0 text-right">
                            <span className="text-slate-500 font-bold mr-4">{t('total_paid')}:</span>
